...
class C_OAS078 extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('m_oas078', 'list_po_model');		
	}
	function load_view()
	{
		...
		$param['submission_list']	= $this->list_po_model->get_list($param);
		...
		$param['list_group']		= $this->list_po_model->get_group_list();
		$param['list_chargecodetype']		= $this->list_po_model->get_chargecodetype_list();
		$param['month_list']		= $this->month_list;	
		$param['employee_list']		= $this->list_po_model->get_employee_list();
		$this->load->view('v_oas078', $param);
		
	}
	function load_form()
	{
			$param['form_detail'] = $this->list_po_model->get_form_detail($form_id);
	}
	...
	{ 
		...
		$search['group_id']			= $this->input->get('PO_group'); 		
		$search['division_id']		= $this->input->get('PO_division'); 	
		$search['chargecodetype_id']	= $this->input->get('PO_cctype'); 	
		$search['chargecode_id']		= $this->input->get('PO_chargecode'); 	
		$search['year']					= $this->input->get('PO_year'); 	
		$search['month']				= $this->input->get('PO_month');	
		$search['PO_status']			= $this->input->get('PO_status');		
	...
	function search() 
	{
		...
		$employeeID 		= $this->user['id']; 	
		$param['this_id']	= $this->user['id']; 
		$param['submission_list']	= $this->list_po_model->get_search_list($employeeID, $search); 		
		$param['list_group']		= $this->list_po_model->get_group_list();  
		$param['detail_admin']		= $this->_get_employee_admin($employeeID);
		$param['detail_akun']		= $this->_get_employee_akun($employeeID);
		$param['detail_dir']		= $this->_get_employee_dir($employeeID);
		$param['detail_pur']		= $this->_get_employee_pur($employeeID);
		if($search['group_id'] != ''){ 	
			$param['list_division'] 	= $this->list_po_model->get_division_list($search['group_id']); 
		}  		
		$param['list_chargecodetype']	= $this->list_po_model->get_chargecodetype_list(); 
		if($search['chargecodetype_id'] != ''){ 	
			$param['list_chargecode']	= $this->list_po_model->get_chargecode_list($search['chargecodetype_id']); 
		} 		 		
	...
	function load_division($group_id = '')
	{
		...
		if($group_id != ''){
			$division_list = $this->list_po_model->get_division_list($group_id);
	...
	function load_chargecode($chargecodetype_id = '')
	{
		...
		if($chargecodetype_id != ''){
			$chargecode_list = $this->list_po_model->get_chargecode_list($chargecodetype_id);
	...
	function download()
	{
		...
		$search = $this->_get_post_data();
		$employeeID = $this->user['id'];
		$submission_list = $this->list_po_model->get_search_list($employeeID, $search);
