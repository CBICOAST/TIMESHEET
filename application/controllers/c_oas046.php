...
class C_OAS046 extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('m_oas046', 'list_ca_model');		
	}
	...
	{ 
		...
		$search['group_id']			= $this->input->get('RCA_group'); 	
		$search['division_id']		= $this->input->get('RCA_division'); 	
		$search['chargecodetype_id']	= $this->input->get('RCA_cctype'); 	
		$search['chargecode_id']		= $this->input->get('RCA_chargecode'); 	
		$search['year']					= $this->input->get('RCA_year'); 	
		$search['month']				= $this->input->get('RCA_month');	
		$search['open']					= $this->input->get('RCA_STAT_OPEN');
		$search['RCA_currency']			= $this->input->get('RCA_currency');
	...
	function download()
	{
		...
		foreach ($submission_list as $keyidx => $valuerecord) 
		{
			...
			$row_array['set_stat'] = $valuerecord['set_stat'];
			$row_array['setl_maker'] = $valuerecord['setl_maker'];
			$row_array['tgl_kwitansi'] = $valuerecord['tgl_kwitansi'];
			$row_array['liability_setl_amount'] =  number_format($valuerecord['liability_setl_amount'],0,',','.');
			$row_array['liability_setl_amount_employee'] =  number_format($valuerecord['liability_setl_amount_employee'],0,',','.');
			$row_array['setl_amount'] =  number_format($valuerecord['setl_amount'],0,',','.');
			$row_array['setl_amount_employee'] =  number_format($valuerecord['setl_amount_employee'],0,',','.');
			$row_array['setl_remarks'] = $valuerecord['setl_remarks'];
			$row_array['setl_remarks_accept'] = $valuerecord['setl_remarks_accept'];
			$row_array['setl_accept_by'] = $valuerecord['setl_accept_by'];
			$row_array['setl_accept_dt'] = $valuerecord['setl_accept_dt'];		
		...
		$objPHPExcel->setActiveSheetIndex(0);
			...
			$objPHPExcel->getActiveSheet()->getStyle('A5:AE5')->applyFromArray($styleArray);
			$objPHPExcel->getActiveSheet()->getStyle('A5:AE5')->applyFromArray($borderArray );
			$objPHPExcel->getActivesheet()->getStyle('A5:AE5')->getFont()->setBold(true);
			...
			$objPHPExcel->getActiveSheet()->getStyle('A5:AE5')->getAlignment()->setWrapText(true);
	...
	private function get_report_header()
	{
		...
		$header['B3'] = date("d-M-Y H:i:s").' ('.$employeeName.')';
			...
			$header['V5'] = "Date of Settlement";
			$header['W5'] = "Receipt Date";
			$header['X5'] = "Liability Company";
			$header['Y5'] = "Liability Employee";
			$header['Z5'] = "Amount Settlement Company";
			$header['AA5'] = "Amount Settlement Employee";
			$header['AB5'] = "Remark Requested Settlement";
			$header['AC5'] = "Remarks Acceptance Settlement";
			$header['AD5'] = " Accepted Settlement by";
			$header['AE5'] = " Accepted Settlement date";
