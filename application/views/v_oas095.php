...
<div class="box-content no-padding">
	<div class="search-fields bs-callout list-title">
		<h2><b> Paid Purchase Request   </b></h2>
		...
		<table border="0" cellpadding="1" cellspacing="1">
				...
				<tr>
					<td colspan="8">
						<button type="submit" class="pull-right btn btn-danger btn-flat btn-slim" id="clear-btn" onclick="change_page(this, 'c_oas095/load_view');">Clear</button>
	...
	<form action="c_oas095/paid_selected" method="post" accept-charset="utf-8">	
	<table class="table table-striped table-bordered table-hover table-heading no-border-bottom">
		...
		<thead>
			<tr>
				...
				<th>Charge Code</th>
				<th>Amount</th>
				<th class="text-center"><button type="submit" class=" btn btn-primary btn-slim" id="paid-selected" >Paid Selected</button><br><input type='checkbox' name='select-all' id='select-all-paidpr' /></th>
			</tr>
		</thead>
		...
		<tbody>
			...
			foreach ($submission_list as $key => $value) { 
				$no++;?>
				<tr class="<?php if (($value['status_id']=='0' && $value['approval']==$this_id)||($value['status_id']=='1' && $detail_akun['id']==$this_id) || ($detail_dir['id']==$this_id && $value['status_id']=='2') || ($detail_pur['id']==$this_id && $value['status_id']=='3')) echo 'warning'; else echo 'Approved'; ?>">
					<td><?= $no ?></td>
					<td class="postit form-number pk" val="<?= $value['pr_id'] ?>"><a href="#"  title="Detail" onclick="change_page(this, 'c_oas077/load_form/<?= $value['pr_id'] ?>')"><?= $value['no_ref'] ?><code><?= $value['status'] ?></code></a></td>
					...
					<input type="checkbox" class="chk" name="checkbox<?php echo $no?>"  value="<?= $value['pr_id'] ?>"/></td>	
					<? }else{?>
					-
					<?}?>
					<input type="text" hidden name="PPR-emp-id<?php echo $no?>"  value="<?= $value['employee_id'] ?>"/>
					<input type="text" hidden name="PPR-no-ref<?php echo $no?>"  value="<?= $value['no_ref'] ?>"/>
	 ...
	 <div class="box-footer clearfix">
                <button type="button" class="pull-left btn btn-default" id="back-btn-paidpr" onclick="change_page(this, 'c_oas065/load_view')">Back...</button>
            </div>
</div>
...
{
	var url="c_oas095/search?"+get_filter_param();
...
{
	var url="c_oas095/download?"+get_filter_param();
...
$(function() { 
	$('#PR_group').change(function(){
        ...
        $.ajax({
            type: "POST",
            url: "c_oas095/load_division/"+sub_unsur_id,
...
$('#select-all-paidpr').click(function(event) {   
    if(this.checked) {
...
$(function() { 
	$('#PR_chargecodetype').change(function(){
        ...
        $.ajax({
            type: "POST",
            url: "c_oas095/load_chargecode/"+sub_unsur_id,
